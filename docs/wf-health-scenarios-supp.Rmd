...
corr_tree_canopy_prop_sqrt_overall=wf_eff_emm_wide_no_outliers %>% 
  mutate(overall=1) %>% 
  group_by(overall) %>% 
  summarise(
    corr_spearman=cor(
  ...
  mutate(measure="tree_canopy_prop_sqrt")
...
corr_tree_canopy_prop_sqrt_biome_name_freq=wf_eff_emm_wide_no_outliers %>% 
...
The stratified scatterplot suggests that in the most urban areas and in the least urban areas, higher tree canopy is associated with a **lower** rate difference. This result is in the expected direction: higher tree canopy, lower effect of wildfire.
In the other two RUCA categories, the association is in the other direction: higher tree canopy is associated with a **higher** rate difference (i.e., suggesting a more harmful effect of wildfire on the outcome as tree canopy rises).
...
corr_tree_canopy_prop_sqrt_ruca_cat=wf_eff_emm_wide_no_outliers %>% 
  group_by(ruca_cat) %>% 
  summarise(
    corr_spearman=cor(
      x=tree_canopy_prop_sqrt,
...
corr_ac_prop_overall=wf_eff_emm_wide_no_outliers %>% 
  mutate(overall=1) %>% 
  group_by(overall) %>% 
  summarise(
    corr_spearman=cor(
  ...
  mutate(measure="ac_prop")

corr_ac_prop_overall %>% 
    kable(digits=2)
...
corr_ac_prop_biome_name_freq=wf_eff_emm_wide_no_outliers %>% 
  group_by(biome_name_freq) %>% 
  summarise(
    corr_spearman=cor(
      x=ac_prop,
...
#| warning: false
#| message: false
corr_ac_prop_ruca_cat=wf_eff_emm_wide_no_outliers %>% 
  group_by(ruca_cat) %>% 
  summarise(
    corr_spearman=cor(
      x=ac_prop,
...
## RD x impervious surfaces x geographic measures
### RD x Proportion impervious surfaces

```{r}
#| echo: false
#| warning: false
#| message: false
glm_rd_100k_quo_pt_imperv_prop=glm(
  rd_100k_quo_pt~imperv_prop,
  family = gaussian,
  na.action = "na.exclude", 
  weights = rd_quo_weight,
  data = wf_eff_emm_wide_no_outliers
...
jtools::summ(
  glm_rd_100k_quo_pt_imperv_prop,
    model.fit=F, #for brevity in the output, dont' show
  digits=3)

corr_imperv_prop_overall=wf_eff_emm_wide_no_outliers %>% 
  mutate(overall=1) %>% 
  group_by(overall) %>% 
  summarise(
    corr_spearman=cor(
  ...
  mutate(measure="imperv_prop")

corr_imperv_prop_overall %>% 
  kable(digits=2)
...
### RD x Proportion impervious surfaces x biome
...
corr_imperv_prop_biome_name_freq=wf_eff_emm_wide_no_outliers %>% 
  group_by(biome_name_freq) %>% 
  summarise(
    corr_spearman=cor(
      x=imperv_prop,
...
corr_imperv_prop_biome_name_freq %>% 
    kable(digits=2)
...
#| warning: false
#| message: false
glm_rd_100k_quo_pt_imperv_prop_int_biome=glm(
  rd_100k_quo_pt~imperv_prop + biome_name_freq +biome_name_freq*imperv_prop,
  family = gaussian,
  na.action = "na.exclude", 
  weights = rd_quo_weight,
  data = wf_eff_emm_wide_no_outliers
...
jtools::summ(
  glm_rd_100k_quo_pt_imperv_prop_int_biome,
      model.fit=F, #for brevity in the output, dont' show
  digits=3)
...
wf_eff_emm_wide %>% 
  ...
  theme_bw()+
    labs(
    y="Rate difference per 100,000 person-days",
    x="Proportion Proportion impervious surfaces"
    )+
  #see here:
  #https://ggplot2.tidyverse.org/reference/facet_grid.html
   theme(strip.text.y = element_text(angle = 0))
...
### RD x Proportion impervious surfaces x rural-urban
...
#| warning: false
#| message: false
corr_imperv_prop_ruca_cat=wf_eff_emm_wide_no_outliers %>% 
  group_by(ruca_cat) %>% 
  summarise(
    corr_spearman=cor(
      x=imperv_prop,
...
#| warning: false
#| message: false
glm_rd_100k_quo_pt_imperv_prop_int_ruca_cat=glm(
  rd_100k_quo_pt~imperv_prop + ruca_cat +ruca_cat*imperv_prop,
  family = gaussian,
  na.action = "na.exclude", 
  weights = rd_quo_weight,
  data = wf_eff_emm_wide_no_outliers
...
jtools::summ(
  glm_rd_100k_quo_pt_imperv_prop_int_ruca_cat,
      model.fit=F, #for brevity in the output, dont' show
  digits=3)
...
## Comparison table of stratified correlations
```{r}
#overall
corr_tree_canopy_prop_sqrt_overall %>% 
...
## Intervention vars x community characteristics
### Tree canopy
...
wf_eff_emm_wide %>% 
  filter(rd_quo_outlier==0) %>% 
  ggplot(aes(y=imperv_prop,x=tree_canopy_prop))+
  geom_point()

wf_eff_emm_wide %>% 
  filter(rd_quo_outlier==0) %>% 
  ggplot(aes(y=imperv_prop,x=tree_canopy_prop_sqrt))+
  geom_point()
